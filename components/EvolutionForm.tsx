    ...
    <div className="w-full max-w-4xl mx-auto glass-effect p-8 rounded-3xl border border-slate-700/50 space-y-12">
      ...
      <section className="space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2 text-white">
          <span className="bg-indigo-600 text-[10px] px-2 py-1 rounded">01</span> Estratégia de Geração
        </h3>
        <div className="grid grid-cols-3 gap-3">
          {[EvolutionType.FROM_SCRATCH, EvolutionType.REPLICATE, EvolutionType.REPLICATE_WITH_CHANGES].map(t => (
            <button key={t} onClick={() => setConfig(prev => ({ ...prev, evolutionType: t }))}
              className={`p-4 rounded-xl border-2 transition-all text-sm font-bold ${config.evolutionType === t ? 'border-indigo-500 bg-indigo-600/20 text-indigo-400' : 'border-slate-800 text-slate-400 hover:border-slate-700'}`}>
      ...
      <section className="space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2 text-white">
          <span className="bg-indigo-600 text-[10px] px-2 py-1 rounded">02</span> Biblioteca de Ativos
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <div className="flex flex-wrap gap-3 p-4 bg-slate-900/40 border border-slate-800 rounded-2xl min-h-[140px]">
              {config.assetImages.map((img, idx) => (
                <div key={idx} className="relative w-24 h-24 rounded-xl overflow-hidden border border-indigo-500/30 group">
              ...
              <button onClick={() => assetInputRef.current?.click()} className="w-24 h-24 border-2 border-dashed border-slate-700 rounded-xl flex flex-col items-center justify-center gap-1 hover:border-indigo-500 hover:bg-indigo-900/10 transition-all text-slate-500 hover:text-indigo-400">
          ...
          <div onClick={() => logoInputRef.current?.click()} className={`relative group cursor-pointer border-2 border-dashed rounded-2xl p-4 flex flex-col items-center justify-center gap-2 ${config.logoImage ? 'border-indigo-500 bg-indigo-900/10' : 'border-slate-700 bg-slate-900/50'}`}>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-bold flex items-center gap-2 text-white">
            <span className="bg-indigo-600 text-[10px] px-2 py-1 rounded">03</span> Headlines e Variações de Copy
          </h3>
          <button onClick={addCopy} className="text-[10px] font-black uppercase text-indigo-400 hover:text-indigo-300 border border-indigo-500/30 px-3 py-1 rounded-lg transition-all">+ Novo Teste de Copy</button>
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-white focus:border-indigo-500 outline-none pr-10"
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 text-indigo-500/40"><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg></div>
                </div>
              </div>
              <div className="space-y-2">
      ...
      <section className="space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2 text-white">
          <span className="bg-indigo-600 text-[10px] px-2 py-1 rounded">04</span> Instruções de Direção de Arte
        </h3>
        <textarea 
          ...
          className="w-full h-32 bg-slate-900/40 border border-slate-800 rounded-2xl p-4 text-sm resize-none text-slate-300 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-600"
        />
      </section>
      ...
      <section className="space-y-6">
        <h3 className="text-xl font-bold flex items-center gap-2 text-white">
          <span className="bg-indigo-600 text-[10px] px-2 py-1 rounded">05</span> Estrutura do Criativo
        </h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button 
            onClick={() => setConfig(prev => ({ ...prev, creativeType: CreativeType.SINGLE }))}
            className={`flex flex-col items-start gap-4 p-6 rounded-2xl border-2 transition-all ${config.creativeType === CreativeType.SINGLE ? 'border-indigo-500 bg-indigo-600/10' : 'border-slate-800 bg-slate-900/30'}`}
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${config.creativeType === CreativeType.SINGLE ? 'bg-indigo-500' : 'bg-slate-800'}`}>
          ...
          <button 
            ...
            className={`flex flex-col items-start gap-4 p-6 rounded-2xl border-2 transition-all ${config.creativeType === CreativeType.CAROUSEL ? 'border-indigo-500 bg-indigo-600/10' : 'border-slate-800 bg-slate-900/30'}`}
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${config.creativeType === CreativeType.CAROUSEL ? 'bg-indigo-500' : 'bg-slate-800'}`}>
        ...
        {isCarousel && (
          <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-8 p-8 bg-slate-900/40 rounded-3xl border border-indigo-500/20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {[3, 5, 7].map(n => (
                    <button key={n} onClick={() => { setIsCustomCards(false); updateCarousel({ cardCount: n }); }} 
                      className={`flex-1 min-w-[60px] py-3 rounded-xl border font-bold text-sm transition-all ${!isCustomCards && config.carouselConfig?.cardCount === n ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'}`}>
                      {n}
                    </button>
                  ))}
                  <button onClick={() => setIsCustomCards(true)} 
                    className={`flex-1 min-w-[100px] py-3 px-4 rounded-xl border font-bold text-sm transition-all ${isCustomCards ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'}`}>
              ...
              <div className="space-y-4">
                ...
                <select 
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-300 outline-none focus:border-indigo-500"
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 border-b border-slate-800">
                <button onClick={() => updateCarousel({ contentOption: 'CENTRAL_IDEA' })} className={`pb-2 px-4 text-xs font-bold transition-all ${config.carouselConfig?.contentOption === 'CENTRAL_IDEA' ? 'text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}>Ideia Central</button>
                <button onClick={() => updateCarousel({ contentOption: 'PER_CARD' })} className={`pb-2 px-4 text-xs font-bold transition-all ${config.carouselConfig?.contentOption === 'PER_CARD' ? 'text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}>Conteúdo por Card</button>
              </div>
              
              {config.carouselConfig?.contentOption === 'CENTRAL_IDEA' ? (
                <textarea 
                  placeholder="A IA criará o roteiro baseado nesta ideia..."
                  className="w-full h-24 bg-slate-950 border border-slate-800 rounded-2xl p-4 text-sm resize-none text-slate-300 focus:border-indigo-500 outline-none"
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scroll">
                  {Array.from({ length: config.carouselConfig!.cardCount }).map((_, i) => (
                    <div key={i} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
                      <span className="text-[10px] text-indigo-400 font-black uppercase">Card {i+1}</span>
                      <input 
                        type="text" placeholder="Headline do Card" 
                        className="w-full bg-transparent border-b border-slate-800 text-sm py-1 focus:border-indigo-500 outline-none text-white"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="space-y-4">
             ...
             <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button key={f.r} onClick={() => addPresetFormat(f.r as AspectRatio, f.l)} className={`px-3 py-2 border rounded-xl text-[10px] font-bold transition-all ${config.formats.some(ff => ff.ratio === f.r) ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700'}`}>
                    {f.l} ({f.r})
                  </button>
                ))}
             </div>
          ...
          <div className="space-y-4">
             ...
             <input 
               ...
               className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-center text-white font-bold outline-none focus:border-indigo-500" 
          ...
          <div className="space-y-4">
             ...
             <div className="grid grid-cols-3 gap-2">
                {["1K", "2K", "4K"].map(sz => (
                  <button key={sz} onClick={() => setConfig(prev => ({ ...prev, size: sz as any }))} className={`py-3 rounded-xl border text-[10px] font-black transition-all ${config.size === sz ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-900 border-slate-800 text-slate-400'}`}>
      ...
      <div className="pt-6">
        <button onClick={() => onGenerate(config)} disabled={isGenerating} className="relative w-full bg-gradient-to-r from-indigo-600 to-purple-600 py-6 rounded-2xl font-black text-xl text-white hover:scale-[1.01] active:scale-95 transition-all shadow-2xl shadow-indigo-500/20 disabled:opacity-50 overflow-hidden group">
