        ...
        <div key={groupId} className="space-y-8">
          {groupId !== 'single' && (
            <div className="flex items-center gap-4">
              <span className="bg-indigo-600/20 text-indigo-400 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest border border-indigo-500/20">Carrossel Estratégico</span>
              <div className="flex-1 h-px bg-slate-800"></div>
            </div>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {groupImages.map((img) => (
              <div key={img.id} className="group relative glass-effect rounded-3xl overflow-hidden border border-slate-700 hover:border-indigo-500/50 transition-all duration-500 flex flex-col">
                <div className={`overflow-hidden bg-slate-950 relative flex items-center justify-center aspect-square`}>
                  ...
                  {img.carouselInfo && (
                    <div className="absolute top-4 left-4 z-20 bg-indigo-600 text-white text-[10px] font-black px-3 py-1 rounded-lg border border-white/10 shadow-xl">
                      CARD {img.carouselInfo.index}/{img.carouselInfo.total}
                    </div>
                  )}

                  <button onClick={() => setEditingId(img.id)} className="absolute top-4 right-4 bg-indigo-600 p-3 rounded-full shadow-lg hover:scale-110 active:scale-90 transition-all z-20">
                ...
                {editingId === img.id && (
                  <div className="absolute inset-0 bg-slate-900/95 p-6 flex flex-col justify-center gap-4 animate-in fade-in duration-300 z-30">
                    <h4 className="font-bold text-indigo-400">Refinar com IA</h4>
                    ...
                    <div className="flex gap-2">
                      <button disabled={isEditing} onClick={() => handleQuickEdit(img.id, img.url)} className="flex-1 bg-indigo-600 py-2 rounded-lg text-sm font-bold text-white disabled:opacity-50">
                ...
                <div className="p-6 space-y-4 mt-auto">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold uppercase tracking-widest text-indigo-400 bg-indigo-500/10 px-2 py-1 rounded">{img.dimensions ? `${img.dimensions.w}x${img.dimensions.h}` : img.aspectRatio}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(img.prompt)} className="bg-indigo-600 hover:bg-indigo-700 py-3 rounded-xl text-xs font-bold transition-all text-white">Prompt</button>
                  </div>
                </div>
              </div>
            ))}
